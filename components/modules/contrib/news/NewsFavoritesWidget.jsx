...
export const config = {
	id: "vactory_news:favorite_listing",
}

const FavoriteNews = ({ data }) => {
	...
	const getCurrentUserFalggedArticles = async () => {
		const response = await drupal.fetch(`api/flagging/all/vactory_news`, {
			withAuth: true,
			method: "GET",
		})
		if (response.ok) {
							...
							<div className="mx-auto grid gap-5 md:grid-cols-2 lg:grid-cols-3">
								{posts.map((post) => (
									<React.Fragment key={post.id}>
										<NewsCard {...post} reloadPage={reloadPage} listingLayout="grid" />
									</React.Fragment>
								))}
							</div>
						) : (
