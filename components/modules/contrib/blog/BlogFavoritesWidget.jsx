...
export const config = {
	id: "vactory_blog:favorite_listing",
}
...
const FavoriteBlog = ({ data }) => {
	...
	const getCurrentUserFalggedArticles = async () => {
		const response = await drupal.fetch(`api/flagging/all/vactory_blog`, {
			withAuth: true,
			method: "GET",
		})
		if (response.ok) {
							...
							<div className="mx-auto grid gap-5 md:grid-cols-2 lg:grid-cols-3">
								{posts.map((post) => (
									<React.Fragment key={post.id}>
										<BlogCard {...post} reloadPage={reloadPage} />
									</React.Fragment>
								))}
							</div>
						) : (
