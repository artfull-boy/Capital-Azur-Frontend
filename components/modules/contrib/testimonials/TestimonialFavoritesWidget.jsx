...
export const config = {
	id: "vactory_testimonials:favorite_listing",
}

const FavoriteTestimonials = ({ data }) => {
	...
	const getCurrentUserFalggedArticles = async () => {
		const response = await drupal.fetch(`api/flagging/all/vactory_testimonials`, {
			withAuth: true,
			method: "GET",
		})
		if (response.ok) {
							...
							<div className="mx-auto grid gap-5 md:grid-cols-2 lg:grid-cols-3">
								{posts.map((post) => (
									<React.Fragment key={post.id}>
										<TestimonialCard {...post} reloadPage={reloadPage} />
									</React.Fragment>
								))}
							</div>
						) : (
