	...
	return (
		<>
			<div className="relative">
				<div ref={sliderRef} className={vclsx("keen-slider", slider[variant].wrapper)}>
					{children}
				</div>
				{loaded && instanceRef.current && (
					<>
						<Arrows left variant={variant} onClick={() => instanceRef.current?.prev()} />

						<Arrows right variant={variant} onClick={() => instanceRef.current?.next()} />
					</>
				)}
