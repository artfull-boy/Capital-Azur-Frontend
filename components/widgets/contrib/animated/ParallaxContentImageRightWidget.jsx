...
export const ParallaxContentImageRight = ({ title, content, cta, image }) => {
	...
	return (
		<div ref={targetRef} className="relative mb-24 px-2 md:mb-44">
			<motion.div
				style={{ y: translateY }}
				className="relative z-10 h-full w-full md:w-1/2 md:py-20"
			>
			...
			<div className="relative right-0 top-0 h-[400px] w-full overflow-hidden rounded-md md:absolute md:h-full md:w-7/12">
				<motion.img
					style={{ scale: scale }}
					src={image}
...
const ParallaxContentImageRightContainer = ({ data }) => {
	const widgetData = {
	...
	return <ParallaxContentImageRight {...widgetData} />
}

export default ParallaxContentImageRightContainer
